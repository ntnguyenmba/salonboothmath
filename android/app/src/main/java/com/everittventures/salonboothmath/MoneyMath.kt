...
object MoneyMath {
    ...
    fun requiredServicesBooth(targetTakeHome: Long, cashTipsCents: Long, cardTipsCents: Long, weeklyRentCents: Long, suppliesCents: Long, extraFeesCents: Long = 0, cardFeeRate: BigDecimal = BigDecimal("0.029"), percentServicesOnCard: BigDecimal = BigDecimal("0.70")): Long {
        val coef = BigDecimal.ONE.subtract(percentServicesOnCard.multiply(cardFeeRate))
        if (coef <= BigDecimal.ZERO) return 0L
        val fixed = BigDecimal.valueOf(cashTipsCents)
            .add(BigDecimal.valueOf(cardTipsCents).multiply(BigDecimal.ONE.subtract(cardFeeRate)))
            .subtract(BigDecimal.valueOf(weeklyRentCents))
            .subtract(BigDecimal.valueOf(suppliesCents))
            .subtract(BigDecimal.valueOf(extraFeesCents))
        val needed = BigDecimal.valueOf(targetTakeHome).subtract(fixed).divide(coef, 0, RoundingMode.HALF_UP)
        return needed.max(BigDecimal.ZERO).longValueExact()
    }

    fun requiredServicesCommission(targetTakeHome: Long, cashTipsCents: Long, cardTipsCents: Long, suppliesCents: Long, cut: BigDecimal, tipOwner: TipOwner, workerPaysCardFees: Boolean, extraFeesCents: Long = 0, cardFeeRate: BigDecimal = BigDecimal("0.029"), percentServicesOnCard: BigDecimal = BigDecimal("0.70"), weeklyRentCents: Long = 0): Long {
        if (cut <= BigDecimal.ZERO) return 0L
        val tips = workerTips(cashTipsCents, cardTipsCents, tipOwner)
        var services = 0L
        repeat(8) {
            val fees = if (workerPaysCardFees) cardFees(services, cardTipsCents, cardFeeRate, percentServicesOnCard) else 0L
            val rhs = BigDecimal.valueOf(targetTakeHome) - BigDecimal.valueOf(tips) + BigDecimal.valueOf(fees) + BigDecimal.valueOf(suppliesCents) + BigDecimal.valueOf(extraFeesCents) + BigDecimal.valueOf(weeklyRentCents)
            val next = rhs.divide(cut, 0, RoundingMode.HALF_UP).max(BigDecimal.ZERO).longValueExact()
            if (kotlin.math.abs(next - services) <= 1L) return next
            services = next
        }
        return services
    }

    fun maxRentToBeatCommission(servicesCents: Long, cashTipsCents: Long, cardTipsCents: Long, suppliesCents: Long, cut: BigDecimal, tipOwner: TipOwner, workerPaysCardFees: Boolean, extraFeesCents: Long = 0, cardFeeRate: BigDecimal = BigDecimal("0.029"), percentServicesOnCard: BigDecimal = BigDecimal("0.70")): Long {
        val commission = commissionTakeHome(servicesCents, cashTipsCents, cardTipsCents, suppliesCents, cut, tipOwner, workerPaysCardFees, extraFeesCents, cardFeeRate, percentServicesOnCard)
        val fees = cardFees(servicesCents, cardTipsCents, cardFeeRate, percentServicesOnCard)
        val gross = servicesCents + cashTipsCents + cardTipsCents
        return maxOf(0L, gross - fees - suppliesCents - extraFeesCents - commission)
    }

    fun breakEvenCutBasisPoints(servicesCents: Long, cashTipsCents: Long, cardTipsCents: Long, suppliesCents: Long, weeklyRentCents: Long, tipOwner: TipOwner, workerPaysCardFees: Boolean, extraFeesCents: Long = 0, cardFeeRate: BigDecimal = BigDecimal("0.029"), percentServicesOnCard: BigDecimal = BigDecimal("0.70")): Int {
        if (servicesCents <= 0) return 0
        val booth = boothTakeHome(servicesCents, cashTipsCents, cardTipsCents, suppliesCents, weeklyRentCents, extraFeesCents, cardFeeRate, percentServicesOnCard)
        val tips = workerTips(cashTipsCents, cardTipsCents, tipOwner)
        val fees = if (workerPaysCardFees) cardFees(servicesCents, cashTipsCents, cardFeeRate, percentServicesOnCard) else 0L
        val rhs = BigDecimal.valueOf(booth) - BigDecimal.valueOf(tips) + BigDecimal.valueOf(fees) + BigDecimal.valueOf(suppliesCents) + BigDecimal.valueOf(extraFeesCents)
        val cut = rhs.divide(BigDecimal.valueOf(servicesCents), 6, RoundingMode.HALF_UP)
        val clamped = cut.max(BigDecimal.ZERO).min(BigDecimal.ONE)
        return clamped.multiply(BigDecimal("10000")).setScale(0, RoundingMode.HALF_UP).intValueExact()
    }
}
