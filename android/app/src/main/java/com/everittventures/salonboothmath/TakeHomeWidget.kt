...
class TakeHomeWidget : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        val amount = formatCents(cents, language)
        val takeHomeLabel = localized.getString(R.string.you_took_home)
        val weekLabel = localized.getString(R.string.this_week)
        provideContent {
            Column(
                ...
                Text(takeHomeLabel, style = TextStyle(color = ColorProvider(Color.White), fontSize = 15.sp, fontWeight = FontWeight.Bold))
                Spacer(GlanceModifier.height(5.dp))
                Text(amount, style = TextStyle(color = ColorProvider(Color.White), fontSize = 32.sp, fontWeight = FontWeight.Bold))
                Spacer(GlanceModifier.height(5.dp))
                Text(weekLabel, style = TextStyle(color = ColorProvider(Color.White), fontSize = 16.sp, fontWeight = FontWeight.Bold))
            }
        }
    }
}
