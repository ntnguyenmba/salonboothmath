...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView { VStack(spacing: 0) { header; fields.padding(.top, 28); result.padding(.top, 32); actions.padding(.top, 24).padding(.bottom, 30) } }.scrollDismissesKeyboard(.interactively)
        ...
        .sheet(isPresented: $showShare) { if let shareImage { ActivityShareView(items: [shareImage, formatCurrency(takeHomeCents), currentWeekStart.formatted(date: .abbreviated, time: .omitted)]) } }
    ...
    private var result: some View {
        VStack(spacing: 10) {
            ...
            Text(formatCurrency(takeHomeCents)).font(Brand.font(52, weight: .heavy)).monospacedDigit().minimumScaleFactor(0.75).lineLimit(1)
            if highRent { Text("br.rentHighHome").font(Brand.font(16, weight: .heavy)).foregroundStyle(Brand.warning).multilineTextAlignment(.center) }
    ...
    private var actions: some View {
        VStack(spacing: 10) {
            ...
            Button { showBreakdown = true } label: { Text("home.breakdown").font(Brand.font(18, weight: .heavy)).foregroundStyle(Brand.berry).frame(maxWidth: .infinity, minHeight: 52) }
            Button { shareCurrentWeek() } label: { Text("home.share").font(Brand.font(17, weight: .heavy)).foregroundStyle(Brand.berry).frame(maxWidth: .infinity, minHeight: 48) }
            Button { requireUnlock(.history) } label: { Text("history.short").font(Brand.font(17, weight: .bold)).foregroundStyle(Brand.berry).frame(maxWidth: .infinity, minHeight: 48) }
            Button { requireUnlock(.compare) } label: { Text("compare.short").font(Brand.font(17, weight: .bold)).foregroundStyle(Brand.berry).frame(maxWidth: .infinity, minHeight: 48) }
        }.padding(.horizontal, Brand.screenPadding)
    }

    private func shareCurrentWeek() { shareImage = ShareCardRenderer.image(takeHomeCents: takeHomeCents, weekStart: currentWeekStart); showShare = shareImage != nil }
    ...
    private func runPendingAction() {
        ...
        switch action {
        case .save: weekStore.save(WeekRecord(weekStart: currentWeekStart, servicesCents: serviceCents, cashTipsCents: cashTipCents, cardTipsCents: cardTipCents, suppliesCents: supplyCents, extraFeesCents: extraFeesCents, payModel: payModel, takeHomeCents: takeHomeCents)); WidgetBridge.updateCurrentWeek(takeHomeCents: takeHomeCents)
...
private struct HomeMoneyField: View {
    let title: String; @Binding var text: String; @FocusState private var focused: Bool
...
struct PaywallView: View {
    @ObservedObject var purchases: PurchaseManager; let completion: (Bool) -> Void
