...
struct HomeView: View {
    ...
    private var payContext: String {
        switch payModel {
        case .booth: return "\(L("pay.booth", language: appLanguage)) · \(formatCurrency(weeklyRentCents, language: appLanguage))/\(L("rent.week", language: appLanguage))"
        case .commission: return String(format: L("home.payContextSplit", table: "Hybrid", language: appLanguage), commissionCutBasisPoints/100, max(0, 100-commissionCutBasisPoints/100))
        case .hybrid: return String(format: L("home.hybridContext", table: "Hybrid", language: appLanguage), formatCurrency(weeklyRentCents, language: appLanguage), commissionCutBasisPoints/100)
        }
    }
    ...
    private var header: some View { ZStack { VStack(spacing:3){Text(isCurrentWeek ? L("home.thisWeek", language: appLanguage) : weekRange(activeWeekStart)).font(Brand.font(19));Text(payContext).font(Brand.font(16)).foregroundStyle(.white)};HStack{if !isCurrentWeek{Button{returnToCurrentWeek()}label:{Image(systemName:"chevron.left").frame(width:48,height:48)}};Spacer();Menu{Picker(language.languageTitle,selection:$appLanguage){ForEach(AppLanguage.allCases){l in Text(l.displayName).tag(l.rawValue)}};Divider();if !purchases.isUnlocked{Button(String(format:String(localized:"paywall.unlockLifetime"),lifetimePrice)){pendingAction=nil;showPaywall=true}};Button("home.share"){shareCurrentWeek()};Button("history.title"){requireUnlock(.history)};Button("compare.title"){requireUnlock(.compare)};Button("settings.title"){showSettings=true}}label:{Image(systemName:"ellipsis.circle.fill").font(.system(size:23,weight:.bold)).frame(width:48,height:48)}}}.foregroundStyle(.white).padding(.horizontal,10).padding(.vertical,12).background(Brand.berry).overlay(alignment:.top){Rectangle().fill(Brand.hotPink).frame(height:4)} }
    private var fields: some View { VStack(spacing:20){HomeMoneyField(title:L("field.services", language: appLanguage),text:$services);HomeMoneyField(title:L("field.tipsCash", language: appLanguage),text:$cashTips);HomeMoneyField(title:L("field.tipsCard", language: appLanguage),text:$cardTips);HomeMoneyField(title:L("field.supplies", language: appLanguage),text:$supplies)}.padding(.horizontal,Brand.screenPadding) }
    private var result: some View { VStack(spacing:8){Text(L("home.youTookHome", language: appLanguage)).font(Brand.font(17)).foregroundStyle(Brand.hotPink);Text(formatCurrency(takeHomeCents)).font(Brand.font(52,weight:.heavy)).monospacedDigit().minimumScaleFactor(0.82).lineLimit(1);if let ratio=highRentRatio,ratio>=Decimal(string:"0.40")!{Text(String(format:String(localized:"br.rentHigh"),NSDecimalNumber(decimal:ratio).doubleValue.formatted(.percent.precision(.fractionLength(0))))).font(Brand.font(16)).foregroundStyle(Brand.warning).multilineTextAlignment(.center)};if let addedTodayGross{Text(copy(en:"Added today · \(formatCurrency(addedTodayGross))",es:"Agregado hoy · \(formatCurrency(addedTodayGross))",vi:"Đã thêm hôm nay · \(formatCurrency(addedTodayGross))")).font(Brand.font(16)).foregroundStyle(Brand.mutedInk)}}.frame(maxWidth:.infinity).padding(.horizontal,Brand.screenPadding) }
    private var actions: some View { VStack(spacing:14){if isCurrentWeek{Button{showAddToday=true}label:{Text(copy(en:"Add today",es:"Agregar hoy",vi:"Thêm hôm nay")).font(Brand.font(18,weight:.heavy)).foregroundStyle(Brand.hotPink).frame(maxWidth:.infinity,minHeight:58).background(Brand.surface).clipShape(RoundedRectangle(cornerRadius:Brand.controlRadius))}};PrimaryButton(title:L("home.save", language: appLanguage)){requireUnlock(.save)};Button{showBreakdown=true}label:{Text(L("home.breakdown", language: appLanguage)).font(Brand.font(18)).foregroundStyle(Brand.ink).frame(maxWidth:.infinity,minHeight:58).background(Brand.surface).clipShape(RoundedRectangle(cornerRadius:Brand.controlRadius))};Button{openCompare()}label:{Text(L("home.compareDeal", table: "Hybrid", language: appLanguage)).font(Brand.font(18)).foregroundStyle(Brand.ink).frame(maxWidth:.infinity,minHeight:58).background(Brand.surface).clipShape(RoundedRectangle(cornerRadius:Brand.controlRadius))}}.padding(.horizontal,Brand.screenPadding) }
    ...
    private func openCompare(){ if purchases.isUnlocked || !didUseFreeCompare { didUseFreeCompare=true; showCompare=true } else { requireUnlock(.compare) } }
...
struct PaywallView: View { @ObservedObject var purchases:PurchaseManager;let completion:(Bool)->Void;private var unlockTitle:String{String(format:String(localized:"paywall.unlockLifetime"),purchases.product?.displayPrice ?? "$9.99")};var body:some View{VStack(alignment:.leading,spacing:18){HStack{Capsule().fill(Brand.hotPink).frame(width:54,height:6);Spacer();Button(String(localized:"paywall.continueFree")){completion(false)}.font(Brand.font(16)).foregroundStyle(Brand.ink)};Text(L("paywall.lifetimeTitle", language: appLanguage)).font(Brand.font(27,weight:.heavy)).lineLimit(2).minimumScaleFactor(0.85);Text(L("paywall.lifetimeBody", language: appLanguage)).font(Brand.font(18)).foregroundStyle(Brand.mutedInk).fixedSize(horizontal:false,vertical:true);PrimaryButton(title:unlockTitle){Task{let ok=await purchases.purchase();if ok{completion(true)}}};Button{Task{await purchases.restore();if purchases.isUnlocked{completion(true)}}}label:{Text(L("paywall.restore", language: appLanguage)).font(Brand.font(17)).foregroundStyle(Brand.ink).frame(maxWidth:.infinity,minHeight:54)};Button{completion(false)}label:{Text(L("paywall.continueFree", language: appLanguage)).font(Brand.font(17)).foregroundStyle(Brand.muted).frame(maxWidth:.infinity,minHeight:54)}}.padding(24).background(Brand.page).foregroundStyle(Brand.ink)} }
