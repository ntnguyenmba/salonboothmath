...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack { Brand.page.ignoresSafeArea(); VStack(spacing: 0) { header; ScrollView { VStack(spacing: 0) { membership.padding(.top, 18); fields.padding(.top, 22); result.padding(.top, 30); actions.padding(.top, 24).padding(.bottom, 32) } }.scrollDismissesKeyboard(.interactively) } }.foregroundStyle(Brand.ink)
    ...
    private var membership: some View {
        HStack(spacing: 14) {
            ...
            if purchases.isUnlocked {
                Image(systemName: "checkmark.seal.fill").font(.system(size: 25, weight: .bold)).foregroundStyle(Brand.hotPink)
            } else {
                Button { pendingAction = nil; showPaywall = true } label: {
