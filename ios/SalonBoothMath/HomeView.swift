...
struct HomeView: View {
    ...
    private enum LockedAction {
        case save
        case compare
        case history
    }

    private var payModel: PayModel {
        PayModel(rawValue: savedPayModel) ?? .booth
    }

    private var rentPeriod: RentPeriod {
        RentPeriod(rawValue: savedRentPeriod) ?? .week
    }

    private var tipOwner: TipOwner {
        TipOwner(rawValue: savedTipOwner) ?? .you
    }

    private var commissionCut: Decimal {
        MoneyMath.rate(fromBasisPoints: commissionCutBasisPoints)
    }

    private var cardFeeRate: Decimal {
        MoneyMath.rate(fromBasisPoints: cardFeeBasisPoints)
    }

    private var servicesOnCardRate: Decimal {
        MoneyMath.rate(fromBasisPoints: servicesOnCardBasisPoints)
    }

    private var taxRate: Decimal {
        MoneyMath.rate(fromBasisPoints: taxBasisPoints)
    }

    private var serviceCents: Int {
        MoneyMath.cents(from: services)
    }

    private var cashTipCents: Int {
        MoneyMath.cents(from: cashTips)
    }

    private var cardTipCents: Int {
        MoneyMath.cents(from: cardTips)
    }

    private var supplyCents: Int {
        MoneyMath.cents(from: supplies)
    }

    private var weeklyRentCents: Int {
        MoneyMath.weeklyRent(cents: rentCents, period: rentPeriod)
    }

    private var grossCents: Int {
        serviceCents + cashTipCents + cardTipCents
    }
    ...
    private var takeHomeCents: Int {
        payModel == .booth ? boothTakeHome : commissionTakeHome
    }
    ...
    private var payContext: String {
        if payModel == .booth {
            return "\(String(localized: "pay.booth")) · \(formatCurrency(weeklyRentCents))/\(String(localized: "rent.week"))"
        }

        let tipsText: String
        switch tipOwner {
        case .you:
            tipsText = String(localized: "tips.you")
        case .house:
            tipsText = String(localized: "tips.house")
        case .split:
            tipsText = String(localized: "tips.split")
        }
        return "\(commissionCutBasisPoints / 100)% · \(tipsText)"
    }

    private var lifetimePrice: String {
        purchases.product?.displayPrice ?? "$4.99"
    }
        ...
        NavigationStack {
            ...
            .navigationDestination(isPresented: $showCompare) {
                CompareView(
                    ...
                    commissionPercent: commissionCutBasisPoints / 100
                )
            }
        ...
        .sheet(isPresented: $showShare) {
            if let shareImage {
                ActivityShareView(items: [
                    shareImage,
                    formatCurrency(takeHomeCents),
                    activeWeekStart.formatted(date: .abbreviated, time: .omitted)
                ])
            }
        }
        .onAppear {
    ...
    private var header: some View {
        ZStack {
            ...
            HStack {
                ...
                Menu {
                    Picker(AppLanguage.current(appLanguage).languageTitle, selection: $appLanguage) {
                    ...
                    if !purchases.isUnlocked {
                        Button("Unlock Lifetime \(lifetimePrice)") {
                            pendingAction = nil
                            showPaywall = true
                        }
                    }
    ...
    private var result: some View {
        VStack(spacing: 8) {
            ...
            if let ratio = highRentRatio,
               ratio >= Decimal(string: "0.40")! {
                Text(
                    String(
                        format: String(localized: "br.rentHigh"),
                        NSDecimalNumber(decimal: ratio)
                            .doubleValue
        ...
        VStack(alignment: .leading, spacing: 11) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: Brand.controlRadius)
                    .stroke(focused ? Brand.hotPink : Brand.line, lineWidth: focused ? 3 : 2)
            )
        }
        .frame(maxWidth: .infinity)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            HStack {
                ...
                Button(AppLanguage.current(appLanguage).cancelTitle) {
                    completion(false)
                }
                .font(Brand.font(16))
                .foregroundStyle(Brand.ink)
            ...
            Text("Salon Booth Math Lifetime")
                .font(Brand.font(28, weight: .heavy))

            Text("Keep the weekly calculator and Breakdown free. Unlock Save Week, History, and Booth vs Commission comparison forever with one purchase.")
                .font(Brand.font(18))
                .fixedSize(horizontal: false, vertical: true)

            VStack(alignment: .leading, spacing: 12) {
                Label("Save every week", systemImage: "checkmark.circle.fill")
                Label("View earnings history", systemImage: "checkmark.circle.fill")
                Label("Compare booth vs commission", systemImage: "checkmark.circle.fill")
                Label("One-time purchase. No subscription.", systemImage: "checkmark.circle.fill")
            }
            .font(Brand.font(16))
            .foregroundStyle(Brand.ink)
            ...
            PrimaryButton(title: unlockTitle) {
                Task {
                    completion(await purchases.purchase())
                }
            }
