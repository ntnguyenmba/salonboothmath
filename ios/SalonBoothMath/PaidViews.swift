...
struct HistoryView: View {
                        ...
                        HStack {
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                if let hours = week.hours, hours > 0 {
                                    Text("\(hours.formatted(.number.precision(.fractionLength(0...1)))) \(label(.hours))")
                                        .font(Brand.font(14))
    ...
    private var metrics: some View {
        VStack(spacing: 12) {
            HStack(spacing: 12) {
                metricCard(label: label(.lastFourWeeks), value: formatCurrency(fourWeekTotal))
                metricCard(label: label(.averageWeek), value: formatCurrency(averageWeek))
            }
            if let averageHourlyCents {
                metricCard(label: label(.averageHourly), value: "\(formatCurrency(averageHourlyCents))/hr")
            }
        }
    }
    ...
    private func metricCard(label: String, value: String) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            Text(label).font(Brand.font(14)).foregroundStyle(Brand.mutedInk)
    ...
    private var trend: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(label(.takeHomeTrend)).font(Brand.font(16))
            HistorySparkline(values: Array(recentWeeks.reversed()).map(\.takeHomeCents))
                .frame(height: 72)
        }
        .padding(16)
        .background(Brand.surface)
    ...
    private func label(_ key: HistoryLabel) -> String {
        ...
        case (.english, .lastFourWeeks): "Last 4 weeks"
        case (.english, .averageWeek): "Average week"
        case (.english, .averageHourly): "Average hourly"
        case (.english, .takeHomeTrend): "Take-home trend"
        case (.english, .hours): "hrs"
        case (.spanish, .lastFourWeeks): "Últimas 4 semanas"
        case (.spanish, .averageWeek): "Promedio semanal"
        case (.spanish, .averageHourly): "Promedio por hora"
        case (.spanish, .takeHomeTrend): "Tendencia del neto"
        case (.spanish, .hours): "h"
        case (.vietnamese, .lastFourWeeks): "4 tuần gần nhất"
        case (.vietnamese, .averageWeek): "Trung bình mỗi tuần"
        case (.vietnamese, .averageHourly): "Trung bình mỗi giờ"
        case (.vietnamese, .takeHomeTrend): "Xu hướng tiền còn lại"
        case (.vietnamese, .hours): "giờ"
        }
    }
}
        ...
        GeometryReader { geometry in
            ...
            Path { path in
                for (index, value) in usable.enumerated() {
                    ...
                    let y = geometry.size.height - (normalized * (geometry.size.height - 12)) - 6
